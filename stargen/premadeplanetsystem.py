...
from .data.tables import OrbitalSpace
...
class PreMadePlanetSystem:
    ...
    def make_gasgiant_arrangement(self):
        user = self.question("Do you want randomly select Gas Giant Arrangement? [Y/N]: ")
        ...
        if user == "Y":
            ...
            #print("self.forbidden:", self.forbidden)
            #print("self.snowline:", self.snowline)
            #print("self.innerforbidden:", self.innerforbidden)
            #print("self.outerforbidden:", self.outerforbidden)
            if self.forbidden == True:
                if self.snowline > self.innerforbidden and self.snowline < self.outerforbidden:
                    self.gasarrangement = 'None'
                else:
                    self.gasarrangement = self.question("What is the arrangement? [None, Conventional, Eccentric, or Epistellar]: ")
            elif self.forbidden == False:
                self.gasarrangement = self.question("What is the arrangement? [None, Conventional, Eccentric, or Epistellar]: ")
    ...
    def createorbits(self):
        ...
        if self.gasarrangement == 'None':
            ...
            if self.premade_world == None:
                ...
                if self.firstgasorbit / self.premade_orbit > 1.4:
                    orbits += [self.firstgasorbit, self.premade_orbit]
                else:
                    print("Orbit of premade is too close to first Gas Giant.")
    ...
    def orbit_inward(self, startorbit, premadeorbit):
        ...
        while allowed:
            ...
            if self.allowed_orbit(neworbit) and oldorbit - neworbit >= 0.15:
                if self.premade_world is not None:
                    #print(f"Premade world is a {self.premade_world}.")
                    #print(startorbit)
                    minorbit = premadeorbit - 0.14
                    maxorbit = premadeorbit + 0.14
                    if  neworbit > minorbit and neworbit < maxorbit:
                        print("New orbit is not valid, continuing.")
                    else:
